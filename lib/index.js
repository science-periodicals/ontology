...
var ontology = require('../ontology.json');
...
let document = jsdom.jsdom(
  fs.readFileSync(path.join(__dirname, 'template.html'), { encoding: 'utf8' })
...
[
  ...
  { id: 'overview-enumeration-members' }
].forEach(x => {
  let terms = ontology.defines
    .filter(term => {
      if (x['@type']) {
        return term['@type'] === x['@type'];
      } else {
        return (
          term['@type'] !== 'rdfs:Class' && term['@type'] !== 'rdf:Property'
        );
      }
    })
    .sort((a, b) => {
...
$article.setAttribute(
  'prefix',
  ['schema', 'sa', 'rdfs', 'rdf', 'owl', 'dc', 'skos']
    .map(x => `${x}: ${ontology['@context'][x]}`)
    .join(' ')
);
    ...
    [
      ...
      values.forEach(value => {
        ...
        let $a = document.createElement(
          prop === 'label' || prop === 'altLabel' ? 'span' : 'a'
        );
        if (prop.charAt(0) !== '@') {
          $a.setAttribute(
            'property',
            ontology['@context'][prop]['@id'] || ontology['@context'][prop]
          );
        }
        ...
        if (isUrl(value)) {
          ...
          if (prefix in ontology['@context']) {
            ...
            href =
              (ontology['@context'][prefix]['@id'] ||
                ontology['@context'][prefix]) + target;
            if (prop.charAt(0) === '@') {
              target = href;
            }
          } else {
            let base =
              ontology['@context']['@base'] || ontology['@context']['@vocab'];
            href = base + value;
            target = value;
          }
        }
