...
let document = jsdom.jsdom(fs.readFileSync('./template.html', {encoding: 'utf8'})).defaultView.document;

let $main = document.createElement('main');
$main.setAttribute('prefix', ['schema', 'sa', 'rdfs', 'rdf', 'owl', 'dc'].map(x => `${x}: ${ontology['@context'][x]}`).join(' '));
...
fs.writeFileSync('../index.html', jsdom.serializeDocument(document), {encoding: 'utf8'});
